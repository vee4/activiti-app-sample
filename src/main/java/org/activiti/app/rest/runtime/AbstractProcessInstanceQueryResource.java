...
public abstract class AbstractProcessInstanceQueryResource {
	...
	public ResultListDataRepresentation getProcessInstances(ObjectNode requestNode) {
		...
		// Group group =
		// identityService.createGroupQuery().groupMember(currentUser.getId()).singleResult();
		...
		if (deploymentKeyNode != null && deploymentKeyNode.isNull() == false) {
			// Results need to be filtered in an app-context. We need to fetch the
			// deployment id for this app and use that in the query
			List<Deployment> deployments = repositoryService.createDeploymentQuery()
					.deploymentKey(deploymentKeyNode.asText()).list();
		...
		if (stateNode != null && !stateNode.isNull()) {
			...
			if ("running".equals(state)) {
				...
				throw new BadRequestException(
						"Illegal state filter value passed, only 'running', 'completed' or 'all' are supported");
			}
		} else {
		...
		// In case we're not on the first page and the size exceeds the page size, we
		// need to do an additional count for the total
